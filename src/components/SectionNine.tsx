...
const SectionNine = ({data}:SectionOneProps) => {
  ...
  return (
    ...
    <h2 className="text-gray-800 mx-auto font-medium text-center  leading-[40px]">{data?.pre_heading}
      <span className="text-primary">{data?.highlightned}</span> {data?.post_heading}
    </h2>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-32 gap-y-8 mt-20 items-center">
      <div>
        <p className="font-light text-sm">{moment(data?.data[0]?.created_at).format("MMM DD, YYYY")}</p>
      ...
      <div className="flex flex-col gap-8">
        <div>
          <p className="font-light text-sm">{moment(data?.data[1]?.created_at).format("MMM DD, YYYY")}</p>
        ...
        <div>
          <p className="font-light text-sm">{moment(data?.data[2]?.created_at).format("MMM DD, YYYY")}</p>
          <p className="text-gray-800 font-semibold mt-2 text-base">
