...
const SectionSix = ({ data }: SectionOneProps) => {
  ...
  return (
    <section className="px-4 md:px-20 lg:px-10 py-14 md:py-24">
      ...
      <h1 className="font-semibold text-[#424242] text-center leading-tight">
              {data?.pre_heading}{" "}
              <span className="text-[#424242]">{data?.highlightned}</span>{" "}
              {data?.post_heading}
            </h1>
            <p className="text-[#666666] font-normal text-center leading-normal mt-4 mb-4">{data?.detail}</p>
      </div>
